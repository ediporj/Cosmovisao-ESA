# Cosmovisão ESA

**ESA** são as iniciais de **Édipo Senna Ázaro** — um ser humano inquieto que, em vez de guardar suas perguntas filosóficas espalhadas por conversas, cadernos e abas mentais abertas, resolveu tentar organizá-las num único lugar.

Este repositório é essa tentativa. Não é uma religião, não é um manifesto, não é doutrina. É o mapa de sentido de uma pessoa específica — feito público porque mapas pessoais às vezes ajudam outros a se orientar.

---

## Consulte o Oráculo

<p align="center">
  <a href="https://ediporj.github.io/Cosmovisao-ESA/oraculo.html">
    <img src="https://img.shields.io/badge/Walter%20Aio-Consultar%20o%20Or%C3%A1culo-ffaa44?style=for-the-badge&labelColor=0c0c15" alt="Consultar o Oráculo Walter Aio"/>
  </a>
</p>

**[→ Walter Aio — Oráculo da Cosmovisão ESA](https://ediporj.github.io/Cosmovisao-ESA/oraculo.html)**

Faça uma pergunta. Receba uma resposta filosófica construída a partir do framework ESA — sobre Deus, morte, amor, karma, livre-arbítrio, consciência e outros 10 temas. Sem inteligência artificial, sem API, sem limite de consultas. Apenas 231 respostas escritas à mão, num oráculo estático que roda no seu navegador.

---

## O que é isto

A Cosmovisão ESA é uma tentativa de unificar, num único sistema coerente, crenças que estavam fragmentadas em "abas" mentais separadas: catolicismo, espiritismo kardecista, budismo, física moderna, tradições orientais. É um **mapa pessoal de sentido** — em expansão contínua.

---

## Do triângulo ao círculo

<p align="center">
  <img src="preview.svg" width="520" alt="Animação Cosmovisão ESA"/>
</p>

O framework parte de três vértices — **Alfa, Beta, Gama** — que formam um triângulo inicial. À medida que novos temas são debatidos entre os vértices, surgem pontos intermediários nomeados (αβ, βγ, αγ, αβ₁, αβ₂...). O triângulo se subdivide progressivamente em direção a um círculo. Os arcos em laranja mostram o espaço que ainda separa o polígono do círculo — a distância que permanece entre o que sabemos e o que é.

**[→ Abrir animação completa](https://ediporj.github.io/Cosmovisao-ESA/Animacao.html)**

---

## Como contribuir

Este framework nasceu de conversas — com pessoas, com textos, com o silêncio. E ainda cresce assim.

Se algo aqui ressoou em você, discordou de você, ou abriu uma pergunta que não tinha antes — isso já é uma contribuição possível. Você pode:

- **Abrir uma Issue** com uma pergunta, objeção ou tensão que o framework ainda não responde bem
- **Propor um novo ponto** do framework — um tema que pertence ao triângulo mas ainda não foi nomeado
- **Sugerir uma resposta** para o Oráculo Walter Aio — se você tem uma forma de dizer algo que o framework já pensa, mas melhor
- **Traduzir** — o framework está em português, mas as perguntas que ele tenta responder não têm língua

Não é preciso concordar com tudo para contribuir. Na verdade, quem discorda com precisão contribui mais do que quem concorda por inércia.

> *"Tudo aqui é hipótese funcional — vale pelo quanto explica e ressoa, não por autoridade."*

---

## Arquivos

### Definições e fundamentos
- [Definições Fundamentais](framework/definicoes-fundamentais.md) — racional, razoável, conflito da mente, individualidade de Deus, existência do ateu
- [Aproximar-se de Deus é Angustiante](framework/angustia-e-deus.md) — lucidez vs. conforto, o paradoxo da busca espiritual

### Vértices do triângulo
- [Estrutura geral](framework/estrutura.md)
- [Ponto Alfa — Santíssima Trindade](framework/ponto-alfa.md)
  - [Alfa 1 — Microcosmo e Macrocosmo](framework/alfa/alfa-1.md)
  - [Alfa 2 — Onipresença de Deus e o Mundo Quântico](framework/alfa/alfa-2.md)
  - [Alfa 3 — A Criança e o Infinito](framework/alfa/alfa-3.md)
- [Ponto Beta — Transcendência Temporal](framework/ponto-beta.md)
- [Ponto Gama — Realidade Dinâmica](framework/ponto-gama.md)

### Pontos intermediários: Alfa × Beta (Alfabetas)
- [Alfabeta 1 — Teoria das Cordas e o Macro/Micro](framework/alfabeta/alfabeta-1.md)
- [Alfabeta 2 — Encarnações ou Desdobramentos?](framework/alfabeta/alfabeta-2.md)
- [Alfabeta 3 — Desdobramentos Hierárquicos em Planos](framework/alfabeta/alfabeta-3.md)
- [Alfabeta 4 — Chakras e Planos de Consciência](framework/alfabeta/alfabeta-4.md)
- [Alfabeta 5 — Caduceu e Kundalini](framework/alfabeta/alfabeta-5.md)
- [Alfabeta 6 — Relações Interpessoais e Karma](framework/alfabeta/alfabeta-6.md)
- [Alfabeta 7 — Regras Físicas de Cada Plano](framework/alfabeta/alfabeta-7.md)

### Pontos intermediários: Alfa 3 × Beta (Alfa3Betas)
- [Alfa3BetaØ — O Despertar de Deus no Caos](framework/alfa3beta/alfa3beta0.md)
- [Alfa3Beta1 — A Consciência do Jogador](framework/alfa3beta/alfa3beta1.md)

### Pontos intermediários: Alfa × Gama (Alfagamas)
- [Alfagama 1 — A Vontade Fragmentada](framework/alfagama/alfagama-1.md)

### Pontos intermediários: Beta × Gama (Betagamas)
- [Betagama 1 — As Trilhas do Tempo Paralelo](framework/betagama/betagama-1.md)

### Referências e influências
- [Helena Blavatsky e o Teosofismo](framework/referencias/blavatsky-teosofia.md)

### Personas
- [Walter Aio — como consultar](personas/walter-aio.md)
- [Thales Aio — como usar](personas/thales-aio.md)
- [A cabeça do Walter Aio — raciocínio interno da persona](personas/A_cabeca_do_Walter-aio.md)

### Oráculo
- [Raciocínio por trás do Oráculo Walter Aio](oraculo/raciocinio-walter-aio.md) — fluxo de pensamento durante a criação das 231 respostas

---

## Como consultar as personas

Veja o guia completo em [como-consultar.md](como-consultar.md).
